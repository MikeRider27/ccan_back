...
create table audit.public_appointment (
...
"date" date NULL,
"time" time(0) NULL,
appointment_type_id int8 NULL,
specialty_id int8 NULL,
patient_id int8 NULL,
appointment_state_id int8 NULL,
...
doctor_id int8 NULL,
deleted bool  NULL,
professional_id int8 NULL,
enable_code varchar(100) NULL
);
...
$BODY$
 ...
 BEGIN   
	...
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_appointment SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_appointment SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
CREATE TRIGGER audit_appointment_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.appointment
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_appointment();


----------------------------------------Tabla appointment_type-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_appointment_type (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
...
CREATE OR REPLACE FUNCTION public.audit_appointment_type()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_appointment_type SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_appointment_type SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_appointment_type_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.appointment_type
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_appointment_type();
...
CREATE OR REPLACE FUNCTION public.audit_eps()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_eps SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_eps SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_eps_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.eps
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_eps();	

 
----------------------------------------Tabla eps_hospital-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_eps_hospital (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
...
CREATE OR REPLACE FUNCTION public.audit_eps_hospital()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_eps_hospital SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_eps_hospital SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_eps_hospital_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.eps_hospital
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_eps_hospital();	
...
----------------------------------------Tabla history_record-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_history_record (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
id int8 NULL,
user_record varchar NULL,
date_record timestamp(0) NULL,
change_record varchar NULL,
json_old jsonb NULL,
json_new jsonb NULL,
...
CREATE OR REPLACE FUNCTION public.audit_history_record()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_history_record SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_history_record SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_history_record_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.history_record
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_history_record();
...
----------------------------------------Tabla medical_document_general-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_medical_document_general (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
...
CREATE OR REPLACE FUNCTION public.audit_medical_document_general()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_medical_document_general SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_medical_document_general SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_medical_document_general_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.medical_document_general
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_medical_document_general();
...
----------------------------------------Tabla medical_monitoring-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_medical_monitoring (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
id int8 NULL,
medical_results_id int8 NULL,
date_monitoring timestamp(0) NULL,
doctor_id int8 NULL,
observation text NULL,
date_create timestamp(0) NULL,
user_create varchar(30) NULL,
...
CREATE OR REPLACE FUNCTION public.audit_medical_monitoring()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_medical_monitoring SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_medical_monitoring SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_medical_monitoring_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.medical_monitoring
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_medical_monitoring();


----------------------------------------Tabla medical_results-------------------------------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_medical_results (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
id int8 NULL,
appointment_id int8 NULL,
date_results timestamp(0) NULL,
description text NULL,
medical_interpretation text NULL,
next_steps text NULL,
birad int8 NULL,
date_sample timestamp(0) NULL,
cie_o_morphology_id int8 NULL,
cie_o_topography_id int8 NULL,
cie_o_tumor_location_id int8 NULL,
laterality int8 NULL,
user_create varchar(30) NULL,
date_create timestamp(0) NULL,
user_modify varchar(30) NULL,
date_modify timestamp(0) NULL,
biopsy_type_result_id int8 NULL
);


---------------------Creación de los trigger functions para auditoría------------------------

CREATE OR REPLACE FUNCTION public.audit_medical_results()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_medical_results SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_medical_results SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_medical_results_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.medical_results
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_medical_results();
...
academic_degree_id int8 NULL,
complementary bool NULL,
complementary_observation varchar(100) NULL,
eps_id int8 NULL,
firstname_2 text NULL,
lastname_2 text NULL,
phone_2 varchar(100) NULL,
affiliation_system_id int8 NULL,
enable_code varchar(100) NULL
);
...
----------------------------------------Tabla previous_studies------------------
---------------------------Creación de las tablas de auditoría-------------------------------

create table audit.public_previous_studies (
audit_event character(1) null,
audit_user varchar(30)null,
audit_date_time timestamp with time zone null,
audit_client_ip inet,
...
CREATE OR REPLACE FUNCTION public.audit_previous_studies()
  RETURNS trigger AS
$BODY$
 DECLARE 
 __DELETE char(6);
 ...
 BEGIN   
	...
	__UPDATE := 'UPDATE';
	if ( TG_OP = __DELETE  ) then 
 		INSERT INTO audit.public_previous_studies SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),OLD.*;
		return OLD;	
	else    
 		INSERT INTO audit.public_previous_studies SELECT substr(TG_OP ,1,1),session_user,current_timestamp,inet_client_addr(),NEW.*;
		return NEW;
	end if; 
 END
 $BODY$
...
-----------------------------Creación del trigger de audit-----------------------------------
CREATE TRIGGER audit_previous_studies_all
  AFTER INSERT OR UPDATE OR DELETE
  ON public.previous_studies
  FOR EACH ROW
  EXECUTE PROCEDURE public.audit_previous_studies();
